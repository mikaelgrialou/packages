...
void Variables::ValuesTreeView::makeTreeView()
{
	...
	row[m_columnsValues.m_col_value] =
            vu::toScientificString(*iter, true);
    }
}
	...
	if (set->isSet()) {
		...
		if (vectorInside.size() >= 2) {
			...
			row[m_columnsVariable.m_col_value] =
			    vu::toScientificString(
				vectorInside[1]->toDouble().value(), true);
...
void Variables::assign(vpz::Condition& condition)
{
	...
	while (iter != refModel->children().end()) {
		...
		value = vu::convert < double >(
		    row.get_value(
			m_variablesTreeView->getColumns()->m_col_value), true);

...
bool PluginPetriNet::create(vpz::AtomicModel& model,
    vpz::Dynamic& /*dynamic*/,
...
void PluginPetriNet::generateObservables(vpz::AtomicModel& model,
    vpz::Observables& observables)
{
...
void PluginPetriNet::generateStructure(vpz::AtomicModel& model)
{
    for (strings_t::const_iterator it = mPetriNet->inputPorts().begin();
         it != mPetriNet->inputPorts().end(); ++it) {
        if (not model.existInputPort(*it)) {
...
bool PluginPetriNet::modify(vpz::AtomicModel& model,
    vpz::Dynamic& /*dynamic*/,
    vpz::Conditions& /*conditions*/,
    vpz::Observables& observables,
